...
def approve_transaction(request, request_id: int):
    ...
    try:
        with transaction.atomic():
            ...
            request.user.save()
            logger.info(
                f"Successfully approved credit request {request_id} for user {request.user.id}",
            )
...
def create_charge(request, data):
    logger.info(
        f"Creating charge sale for user {request.user.id}, amount: {data.amount}, phone: {data.phone_number_id}",
    )

    if request.user.credit < data.amount:
        logger.warning(
            f"Insufficient credit for user {request.user.id}. Required: {data.amount}, Available: {request.user.credit}",
        )
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(f"Successfully created charge sale for user {request.user.id}")
    except Exception as e:
